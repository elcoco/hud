 ...
 */
static GDateTime *gtk_clock_get_time(GtkClock *clock)
{
    if (clock->timezone)
...
#define BLACK ((GdkRGBA) { 0, 0, 0, 1 })
     ...
     * the aspect ratio.
     */
    gtk_snapshot_scale(snapshot, MIN(width, height) / 100.0,
    ...
    gsk_rounded_rect_init_from_rect(&outline,
				    &GRAPHENE_RECT_INIT(-50, -50, 100,
							100), 50);
    gtk_snapshot_append_border(snapshot, &outline, (float[4]) { 4, 4, 4,
			       4 },
			       (GdkRGBA[4]) { BLACK, BLACK, BLACK,
			       BLACK });
     ...
     */
    gtk_snapshot_save(snapshot);
    gtk_snapshot_rotate(snapshot,
			30 * g_date_time_get_hour(time) +
			0.5 * g_date_time_get_minute(time));
    gsk_rounded_rect_init_from_rect(&outline,
				    &GRAPHENE_RECT_INIT(-2, -23, 4, 25),
				    2);
...
static int gtk_clock_get_intrinsic_width(GdkPaintable *paintable)
{
    return 100;
}

static int gtk_clock_get_intrinsic_height(GdkPaintable *paintable)
{
    return 100;
}
 ...
 */
static void gtk_clock_paintable_init(GdkPaintableInterface *iface)
{
 ...
 */
static gboolean gtk_clock_tick(gpointer unused)
{
    GSList *l;
...
static void gtk_clock_stop_ticking(GtkClock *self)
{
...
static void gtk_clock_start_ticking(GtkClock *self)
{
...
static void gtk_clock_class_init(GtkClockClass *klass)
{
	...
	g_param_spec_string("location", NULL, NULL, NULL,
			    G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY);
    properties[PROP_TIME] =
	g_param_spec_boxed("time", NULL, NULL, G_TYPE_DATE_TIME,
			   G_PARAM_READABLE);
    properties[PROP_TIMEZONE] =
	g_param_spec_boxed("timezone", NULL, NULL, G_TYPE_TIME_ZONE,
			   G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY);

    g_object_class_install_properties(gobject_class, N_PROPS, properties);
}

static void gtk_clock_init(GtkClock *self)
{
    gtk_clock_start_ticking(self);
}

static GtkClock *gtk_clock_new(const char *location, GTimeZone *_tz)
{
    GtkClock *result;

    result = g_object_new(GTK_TYPE_CLOCK,
			  "location", location, "timezone", _tz, NULL);
...
static GListModel *create_clocks_model(void)
{
    GListStore *result;
    GtkClock *clock;
...
static char *convert_time_to_string(GObject *image,
				    GDateTime *time, gpointer unused)
{
...
setup_listitem_cb(GtkListItemFactory *factory, GtkListItem *list_item)
{
     ...
     */
    expression =
	gtk_constant_expression_new(GTK_TYPE_LIST_ITEM, list_item);
    clock_expression =
	gtk_property_expression_new(GTK_TYPE_LIST_ITEM, expression,
				    "item");
    ...
    expression = gtk_property_expression_new(GTK_TYPE_CLOCK,
					     gtk_expression_ref
					     (clock_expression), "time");
    expression =
	gtk_cclosure_expression_new(G_TYPE_STRING, NULL, 1,
				    (GtkExpression *[1]) {
				    expression},
...
GtkWidget *do_listview_clocks(GtkWidget *do_widget)
{
	...
	g_signal_connect(factory, "setup", G_CALLBACK(setup_listitem_cb),
			 NULL);
	...
	gtk_scrollable_set_hscroll_policy(GTK_SCROLLABLE(gridview),
					  GTK_SCROLL_NATURAL);
	gtk_scrollable_set_vscroll_policy(GTK_SCROLLABLE(gridview),
					  GTK_SCROLL_NATURAL);
